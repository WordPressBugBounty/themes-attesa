...
function attesa_body_classes( $classes ) {
	...
	// Add a class if Ajax add to cart is enabled
	if (function_exists( 'is_woocommerce' ) && get_option('woocommerce_enable_ajax_add_to_cart') == 'yes' && is_product() && attesa_options('_woocommerce_ajax_add_to_cart', '') == 1 ) {
		$classes[] = 'attesa-ajax-add-to-cart';
	}
	...
	// Adds a class if the header is sticky
	if (apply_filters( 'attesa_sticky_header_scroll', attesa_options('_sticky_header', '1') )) {
		$classes[] = 'headerSticky';
	}
	if (apply_filters( 'attesa_sticky_header_scroll_mobile', attesa_options('_sticky_header_mobile', '') )) {
		$classes[] = 'headerMobileSticky';
	}
	
	// Adds a class if top bar is visible on mobile
	if (apply_filters( 'attesa_show_top_bar_mobile', attesa_options('_show_topbar_mobile', '1') )) {
		$classes[] = 'topBarMobile';
	}
	
	// Adds a class when the topbar is visile
	if (apply_filters( 'attesa_show_top_bar', attesa_options('_show_topbar', '1')) ) {
		$classes[] = 'attesa-topbar';
	}
...
if( ! function_exists('attesa_google_fonts_heading')){
	function attesa_google_fonts_heading() {
		...
		asort($attesa_google_fonts_heading);
...
if( ! function_exists('attesa_google_fonts_text')){
	function attesa_google_fonts_text() {
		...
		asort($attesa_google_fonts_text);
...
if( ! function_exists('attesa_standard_and_custom_fonts')){
	function attesa_standard_and_custom_fonts() {
		...
		asort($attesa_standard_fonts_array);
...
if( ! function_exists('attesa_font_awesome_5_icon_array_cart')){
	function attesa_font_awesome_5_icon_array_cart(){
		return array("fas fa-luggage-cart", "fas fa-shopping-cart", "fas fa-cart-arrow-down", "fas fa-cart-plus", "fab fa-opencart", "fas fa-shopping-bag");
	}
}
...
if ( ! function_exists( 'attesa_get_schema_markup' ) ) {
	function attesa_get_schema_markup($position) {
		...
		if ('body' == $position) {
			if ( is_singular() ) {
				$markup = 'itemscope itemtype="https://schema.org/WebPage"';
			} else {
				$markup = 'itemscope itemtype="https://schema.org/Article"';
			}
		}
		elseif ( 'header' == $position ) {
			$markup = 'itemscope itemtype="https://schema.org/WPHeader"';
		}
		elseif ( 'sidebar' == $position ) {
			$markup = 'itemscope itemtype="https://schema.org/WPSideBar"';
		}
		elseif ( 'footer' == $position ) {
			$markup = 'itemscope itemtype="https://schema.org/WPFooter"';
		}
		elseif ( 'site-title' == $position ) {
			$markup = 'itemscope itemtype="https://schema.org/WebSite"';
		}
		...
		elseif ( 'post-date' == $position ) {
			$markup = 'itemscope itemtype="https://schema.org/Date"';
		}
		elseif ( 'data-pub' == $position ) {
			$markup = 'itemprop="datePublished"';
		}
		elseif ( 'data-mod' == $position ) {
			$markup = 'itemprop="dataModified"';
		}
		...
		elseif ( 'site-navigation' == $position ) {
			$markup = 'itemscope itemtype="https://schema.org/SiteNavigationElement"';
		}
		elseif ( 'portfolio-main' == $position ) {
			$markup = 'itemscope itemtype="https://schema.org/ImageGallery"';
		}
		elseif ( 'portfolio-single' == $position ) {
			$markup = 'itemprop="associatedMedia" itemscope itemtype="https://schema.org/ImageObject"';
		}
...
function attesa_logo_on_scroll() {
	...
	if ( has_custom_logo() && $logoOnScroll && $attesa_stickyHeader && $headerFormat != 'featuredtitle' ) {
		$logoAlternative = 'data-logo-on-scroll="' . esc_url($logoOnScroll) . '"';
	} else {
		$logoAlternative = false;
	}
	echo $logoAlternative; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
...
/* Ajax call for the add-to-cart option in product pages */
add_action('wp_ajax_attesa_woocommerce_ajax_add_to_cart', 'attesa_woocommerce_ajax_add_to_cart'); 
add_action('wp_ajax_nopriv_attesa_woocommerce_ajax_add_to_cart', 'attesa_woocommerce_ajax_add_to_cart');          
function attesa_woocommerce_ajax_add_to_cart() {
	check_ajax_referer( 'attesa_woocommerce_ajax_add_to_cart_nonce', 'security' );
    $product_id = apply_filters('attesa_woocommerce_add_to_cart_product_id', absint($_POST['product_id']));
    $quantity = empty($_POST['quantity']) ? 1 : wc_stock_amount($_POST['quantity']);
    $variation_id = absint($_POST['variation_id']);
    $passed_validation = apply_filters('attesa_woocommerce_add_to_cart_validation', true, $product_id, $quantity);
    $product_status = get_post_status($product_id); 
    if ($passed_validation && WC()->cart->add_to_cart($product_id, $quantity, $variation_id) && 'publish' === $product_status) {
    	do_action('attesa_woocommerce_ajax_added_to_cart', $product_id);
        WC_AJAX :: get_refreshed_fragments(); 
    } else { 
        $data = array(
            'error' => true,
            'product_url' => apply_filters('attesa_woocommerce_cart_redirect_after_error', get_permalink($product_id), $product_id)
        );
