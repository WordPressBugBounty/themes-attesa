...
function attesa_scripts() {
	...
	if ( class_exists( 'WooCommerce' ) ) {
		...
		if (is_product() && (attesa_options('_woocommerce_stickybar', '1') || attesa_options('_woocommerce_ajax_add_to_cart', '') ) ) {
			wp_enqueue_script( 'attesa-woocommerce', get_template_directory_uri() . '/js/woocommerce'.$min.'.js', array('jquery'), wp_get_theme()->get('Version'), true );
			wp_localize_script(
				'attesa-woocommerce',
				'attesaWooSettings',
				array(
					'error' => esc_html__('Error', 'attesa'),
					'added_to_cart' => esc_html__('Added to cart', 'attesa'),
					'nonce' => wp_create_nonce('attesa_woocommerce_ajax_add_to_cart_nonce'),
				)
			);
		}
	}
	...
	if( attesa_options('_show_loader', '') == 1 ) {
		wp_enqueue_style( 'attesa-loader', get_template_directory_uri() .'/css/loader'.$min.'.css', array(), wp_get_theme()->get('Version'));
	}
	if (apply_filters( 'attesa_show_top_bar', attesa_options('_show_topbar', '1'))) {
		wp_enqueue_style( 'attesa-top-bar', get_template_directory_uri() .'/css/top-bar'.$min.'.css', array(), wp_get_theme()->get('Version'));
	}
	...
	if (attesa_options('_choose_icon_pack', 'font_awesome_five') == 'font_awesome_four') {
		wp_enqueue_style( 'attesa-font-awesome-4-comp', get_template_directory_uri() .'/css/fa-comp'.$min.'.css', array(), wp_get_theme()->get('Version'));
		...
		wp_enqueue_style( 'font-awesome-5-all', get_template_directory_uri() .'/css/all'.$min.'.css', array(), '5.15.3');
	} elseif (attesa_options('_choose_icon_pack', 'font_awesome_five') == 'font_awesome_five_comp') {
		wp_enqueue_style( 'font-awesome-5-all', get_template_directory_uri() .'/css/all'.$min.'.css', array(), '5.15.3');
		wp_enqueue_style( 'font-awesome-4-shim', get_template_directory_uri() .'/css/v4-shims'.$min.'.css', array(), '5.15.3');
	}
	...
	if (empty($disableGoogleFonts)) {
		...
		$font_families = array(
			$fontNameText.':wght@400;700',
			$fontNameHead.':wght@400;700'
		);
		if (is_rtl()) {
			$query_args = array(
		...
		if ( attesa_options('_google_fonts_locally', '') == 1 ) {
			require_once get_theme_file_path( 'inc/wptt-webfont-loader.php' );
			wp_enqueue_style( 'attesa-googlefonts', wptt_get_webfont_url(add_query_arg( $query_args, 'https://fonts.googleapis.com/css2' ) ), array(), null );
		} else {
			wp_enqueue_style( 'attesa-googlefonts', add_query_arg( $query_args, "//fonts.googleapis.com/css2" ), array(), null );
		}
	}
	...
	if ( is_singular() && get_option( 'thread_comments' ) ) {
		if (comments_open()) {
			wp_enqueue_script( 'attesa-comment-reply', get_template_directory_uri() . '/js/comment-reply'.$min.'.js', array(), wp_get_theme()->get('Version'), true );
		}
		if (comments_open() || get_comments_number() > 0 ) {
			wp_enqueue_style( 'attesa-comments', get_template_directory_uri() .'/css/comments'.$min.'.css', array(), wp_get_theme()->get('Version'));
		}
	}
}
...
if ( function_exists( 'EDD' ) ) {
	require get_template_directory() . '/inc/third/edd.php';
}
